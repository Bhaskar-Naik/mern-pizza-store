...
import { getAllUsers, makeUserAdmin, removeUserAdmin, deleteUser } from '../../services/authService';
...
const ManageUsers = () => {
  ...
  const isSuperAdmin = currentUser?.email === MAIN_ADMIN_EMAIL;
  ...
  const handleDeleteUser = async (user) => {
    const result = await Swal.fire({
      title: 'Delete User?',
      html: `Are you sure you want to PERMANENTLY delete <b>${user.name}</b>?<br/>This action cannot be undone.`,
      icon: 'error',
      showCancelButton: true,
      confirmButtonColor: '#d33',
      cancelButtonColor: '#6c757d',
                  ...
                  <tr key={user._id} style={{ borderBottom: '1px solid #eee' }}>
                    ...
                    <td style={{ padding: '12px 16px' }}>
                      <div className="d-flex align-items-center">
                        ...
                        {isSuperAdmin && user.email !== MAIN_ADMIN_EMAIL && (
                          <button
